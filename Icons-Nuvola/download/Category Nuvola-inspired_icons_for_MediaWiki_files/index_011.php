...
var tab_purge = 'purge';
includePage( 'MediaWiki:Extra-tabs.js/' + wgUserLanguage );
...
function global_add_image_tabs(imagetitle)
{
  ...
  global_append_tab('/w/index.php?title=' + tab_ns_image + imagetitle + '&action=purge', tab_purge, 'ca-purge');
...
function global_add_special_tabs()
{
  if (wgPageName == "Special:Blockip")
  {
    ...
    if (username != '')
    {
      global_append_tab('/w/index.php?title=' + tab_ns_special + 'Log&type=block&page=' + tab_ns_user + encodeURIComponent (username), tab_blocklog, 'ca-blocklog');
    }
  }
...
function addCheckUsageLink()
{
 ...
 cu.setAttribute('href', 'http://tools.wikimedia.de/~daniel/WikiSense/CheckUsage.php?w=_100000&i=' + encodeURIComponent(wgTitle));
